...
int	data_find(t_game *game)
{
	...
	{
		if (parse_check(game->info.map[i], &game->info.no, "NO ", &checker))
			return (1);
		else if (parse_check(game->info.map[i], &game->info.so, "SO ", &checker))
			return (1);
		else if (parse_check(game->info.map[i], &game->info.we, "WE ", &checker))
			return (1);
		else if (parse_check(game->info.map[i], &game->info.ea, "EA ", &checker))
			return (1);
		else if (parse_check(game->info.map[i], &game->info.c_tmp, "C ", &checker))
			return (1);
		else if (parse_check(game->info.map[i], &game->info.f_tmp, "F ", &checker))
			return (1);
		else if (!checker && map_check(game->info.map[i]))
			break ;
		checker = 0;
		i++;
	}
